...
public class PO_PrivateAdminView extends PO_PrivateView {
	...
	public static void checkChangeIdiomAddOffer(WebDriver driver) {
		SeleniumUtils.EsperaCargaPagina(driver, "text", p.getString("user.listUsers.title", PO_Properties.SPANISH),
				getTimeout());
		SeleniumUtils.EsperaCargaPagina(driver, "text",
				p.getString("user.listUsers.description", PO_Properties.SPANISH), getTimeout());
		SeleniumUtils.EsperaCargaPagina(driver, "text", p.getString("user.listUsers.title.name", PO_Properties.SPANISH),
				getTimeout());
		changeIdiom(driver, "btnEnglish");
		SeleniumUtils.EsperaCargaPagina(driver, "text", p.getString("user.listUsers.title", PO_Properties.ENGLISH),
				getTimeout());
		SeleniumUtils.EsperaCargaPagina(driver, "text",
				p.getString("user.listUsers.description", PO_Properties.ENGLISH), getTimeout());
		SeleniumUtils.EsperaCargaPagina(driver, "text", p.getString("user.listUsers.title.name", PO_Properties.ENGLISH),
				getTimeout());
	...
	/**
	 * Comprobar que un usuario, dado su email, está presente en la página con el
	 * listado de usuarios
	 * 
	 * @param driver
	...
	/**
	 * Comprobar que todos los usuarios pasados por lista están mostrándose y no
	 * están repetidos
	 * 
	 * @param driver
	 * @param emails con una lista de string que contiene los identificadores
	 *               (emails)
	 */
	...
	private static void checkIndividualPosition(int listSize, int pos) {
		assertTrue("La posición " + pos + " indicada no existe",
				pos < listSize && (pos >= PRIMERO_LISTA || pos == ULTIMO_LISTA));
	}
