...
public class SeleniumUtils {
	/**
	 * Aborta si el "id" no está presente en la página actual
	 * 
	 * @param driver: apuntando al navegador abierto actualmente.
	 * @param texto: id a buscar
	 */
	static public void idPresentePagina(WebDriver driver, String id, int timeout) {
		List<WebElement> list = driver.findElements(By.xpath("//*[contains(@id,'" + id + "')]"));
		assertTrue("Id " + id + " no localizada!", list.size() > 0);
	}

	/**
	 * Aborta si el "id" está presente en la página actual
	 * 
	 * @param driver: apuntando al navegador abierto actualmente.
	 * @param texto: id a buscar
	 */
	static public void idNoPresentePagina(WebDriver driver, String id, int timeout) {
		List<WebElement> list = driver.findElements(By.xpath("//*[contains(@id,'" + id + "')]"));
		assertTrue("Id " + id + " aun presente!", list.size() == 0);
	}

	/**
	 * Aborta si el "id" está presente en la página actual tras timeout segundos.
	 * 
	 * @param    driver: apuntando al navegador abierto actualmente.
	 * @param id con el id a buscar
	 * @param    timeout: el tiempo máximo que se esperará por la aparición del
	 *           texto a buscar
	 */
	public static void esperaCargaPaginaNoIdPresente(WebDriver driver, String id, int timeout) {
		Boolean resultado = (new WebDriverWait(driver, timeout))
				.until(ExpectedConditions.invisibilityOfElementLocated(By.xpath("//*[contains(@id,'" + id + "')]")));

		assertTrue(resultado);
	}

	/**
	 * Aborta si el "id" no está presente en la página actual tras timeout segundos.
	 * 
	 * @param    driver: apuntando al navegador abierto actualmente.
	 * @param id con el id a buscar
	 * @param    timeout: el tiempo máximo que se esperará por la aparición del
	 *           texto a buscar
	 */
	public static void esperaCargaPaginaIdPresente(WebDriver driver, String id, int timeout) {
		WebElement resultado = (new WebDriverWait(driver, timeout))
				.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//*[contains(@id,'" + id + "')]")));
		assertTrue(resultado != null);
		List<WebElement> elementos = driver.findElements(By.xpath("//*[contains(@id,'" + id + "')]"));
		assertTrue("Id " + id + " no localizada!", !elementos.isEmpty());
	}
